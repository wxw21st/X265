...
static void xPutBits32(xBitStream *pBS, UInt32 uiBits)
{
...
static void xPutBits(xBitStream *pBS, UInt32 uiBits, Int nNumBits)
{
...
static void xWriteAlignZero(xBitStream *pBS)
{
    Int nShift = (8 - pBS->nCachedBits) & 7;
    xPutBits(pBS, 0, nShift);
}

static void xWriteAlignOne(xBitStream *pBS)
{
    Int nShift = (32 - pBS->nCachedBits) & 7;
    xPutBits(pBS, (1<<nShift)-1, nShift);
}

static void xWriteByteAlignment(xBitStream *pBS)
{
    xPutBits(pBS, 1, 1);
    xWriteAlignZero(pBS);
}

static void xWriteRBSPTrailingBits(xBitStream *pBS)
{
    xPutBits(pBS, 1, 1);
    xWriteAlignZero(pBS);
}
...
// ***************************************************************************
static void xWriteCode( xBitStream *pBS, UInt32 uiCode, UInt32 uiLength )
{
    assert ( uiLength > 0 );
    xPutBits( pBS, uiCode, uiLength );
}

static void xWriteUvlc( xBitStream *pBS, UInt32 uiCode )
{
...
static void xWriteSvlc( xBitStream *pBS, Int32 iCode )
{
    UInt uiCode;
...
static void xWriteFlag( xBitStream *pBS, UInt32 uiCode )
{
